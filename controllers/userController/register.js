...
async function handleRegister(req, res) {
  try {
    const { username, email, number, password, newsletter, checkPassword } =
      req.body;
    ...
    const numberFound = await User.findOne({ number });
    if (numberFound !== null) {
      throw { status: 409, message: "Le Numéro existe Déjà" };
    }
    ...
    if (err.http_code === 401) {
      return res
        .status(500)
        .json({ message: "Internal Service Error: Invalid key" });
    }
    res
